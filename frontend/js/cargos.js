...
function poblarSelectores() {
  ...
  selProf.innerHTML = '<option value="">— Selecciona un profesor —</option>' +
    todosProfesores.sort((a, b) => a.apellidos.localeCompare(b.apellidos))
      .map(p => `<option value="${p.id}">${p.apellidos}, ${p.nombre} (${p.puesto})</option>`).join('');
  const selCargo = document.getElementById('asig-cargo-cargo');
  selCargo.innerHTML = '<option value="">— Selecciona un cargo —</option>' +
    todosCargos.map(c => `<option value="${c.id}" data-horas="${c.horas}">${c.nombre} (${c.horas}h)</option>`).join('');
}
...
async function guardarCargo() {
  ...
  try {
    ...
    renderCargos();
    poblarSelectores();
  } catch (err) { showAlert(err.error || 'Error al guardar.', 'error'); }
}
...
async function guardarAsignacionCargo() {
  ...
  try {
    if (id) {
      await fetch(`${API_BASE}/cargos/asignaciones/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
      showAlert('Asignación actualizada.');
    } else {
      await fetch(`${API_BASE}/cargos/asignaciones`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
